...
class BacktestEngine:
    ...
    def _prefetch_data(self) -> None:
        ...
        for ticker in self._tickers:
            get_prices(ticker, start_date_str, self._end_date, interval=self._interval, interval_multiplier=self._interval_multiplier, include_snapshot=False)
        ...
        get_prices("SPY", self._start_date, self._end_date, interval=self._interval, interval_multiplier=self._interval_multiplier, include_snapshot=False)
    ...
    def run_backtest(self) -> PerformanceMetrics:
        ...
        for current_date in dates:
            ...
            try:
                ...
                for ticker in self._tickers:
                    try:
                        price_data = get_price_data(ticker, previous_date_str, current_date_str, include_snapshot=False)
                        if price_data.empty:
                            missing_data = True
                            break
            ...
            rows = self._results.build_day_rows(
                ...
                benchmark_return_pct=self._benchmark.get_return_pct("SPY", self._start_date, current_date_str, interval=self._interval, interval_multiplier=self._interval_multiplier),
            )
