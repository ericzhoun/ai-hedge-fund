...
def detect_htf_trend(htf: pd.DataFrame, ema_span: int = 20) -> dict:
    ...
    Requires at least 6 weekly bars (for swing structure). EWM works with any
    number of bars but benefits from more warmup — pass a longer date range
    (--start-date 1+ year back) for a fully warmed-up 20-week EMA.
    """
    if len(htf) < 6:
        return {"direction": "neutral", "ema_slope": 0.0, "above_ema": None, "structure": "insufficient"}

    close = htf["close"]
    # Use whatever span fits; EWM is valid with any n, just less warmed-up with few bars
    effective_span = min(ema_span, len(htf))
    ema = close.ewm(span=effective_span, adjust=False).mean()
    # Slope over last 5 bars, or as many as available
    slope_lookback = min(5, len(htf) - 1)
    ema_slope = float((ema.iloc[-1] - ema.iloc[-1 - slope_lookback]) / max(abs(ema.iloc[-1 - slope_lookback]), 1e-9))
